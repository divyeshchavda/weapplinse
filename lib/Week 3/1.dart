...
void main(){
  runApp(MaterialApp(home: Task321(),));
}
                    ...
                    child: TextFormField(
                      validator: (a) {
                        ...
                        if (!RegExp(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$').hasMatch(a!)) {
                          return """Password is Must Contain 8 character,
                    ...
                    child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.black),
                        onPressed: () async {
                          if (t.currentState?.validate() ?? false) {
                            if (file != null) {
                              final imageBytes = await file!.readAsBytes(); // Convert image file to bytes
                              setState(() {
                              ...
                              db.add(name, city, pass, imageBytes); // Pass binary data to the database
